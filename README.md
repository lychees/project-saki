...
- `GET /profile?cf=<handle>&atc=<user>&luogu=<uid>&vj=<user>`：选手档案聚合
  ...
  按 problem.tags 统计的 AC 题数，取样最近 10000 提交）；AtCoder 三级回退
  （直连 history/json → r.jina.ai → **kenkoooo AtCoder Problems API 兜底**，
  atcoder.jp 封锁数据中心 IP，kenkoooo 的 Rated Point Sum / AC 数保证卡片永远
  有数据，rating 仅在回退成功时显示）；Luogu 用户页（__client_id cookie 反爬
  两次握手 + 内嵌数据提取并 JSON 解码 \uXXXX：name/ranking/ccfLevel/通过数，
  注意低频访问，突发请求会被 403）；vjudge 无用户接口，仅统计最近
...
- **rating 卡片**：Codeforces 按段位着色（灰<1200 绿<1400 青<1600 蓝<1900
  ...
  （atcoder.jp 不可达时降级显示 kenkoooo 的 Rated Point Sum + AC 数/排名）；
  Luogu 显示 ccfLevel / 排名 / 通过数；vjudge 显示近期窗口 AC 数。
  **卡片头部的账号名可点击**（下划线/hover 变色），跳转对应平台个人主页。
  获取失败的平台显示「暂未获取」。
